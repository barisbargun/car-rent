import { CarouselPost } from '@repo/typescript-config/types/api'
...
const url = getPath('carousels')
const api = db.carousel
export const carouselsHandlers = [
  http.get(url, async () => {
    await networkDelay()
    try {
      const carousels = api.getAll().map((carousel) => {
        const img = populateWithImage(carousel.img)
      ...
      return HttpResponse.json(carousels)
    } catch (error: any) {
  ...
  http.post(url, async ({ request, cookies }) => {
    ...
    try {
      ...
      const data = (await request.json()) as CarouselPost
      const result = api.create({
        ...data,
      })
      await persistDb('carousel')
